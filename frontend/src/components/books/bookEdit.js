...
const BookEdit = (props) => {
    ...
    const onFormSubmit = (e) => {
        ...
        const authorId = formData.author !== 1 ? formData.author : props.book.author.id;
                                ...
                                {props.categories.map(term => {
                                    if (props.book.category !== undefined && props.book.category === term)
                                        return <option selected={props.book.category}
                                                       value={term}>{term.charAt(0).toUpperCase() + term.slice(1).toLowerCase()}</option>
                                    else return <option value={term}>{term.charAt(0).toUpperCase() + term.slice(1).toLowerCase()}</option>
                                })}
                            </select>
                        </div>
                        <div className="form-group">
